...
use App\View\Components\Bookmark\GroupCard;
...
it('shows bookmark group name', function ($name) {
    ...
    $component = $this->component(GroupCard::class, [
...
it('shows a maximum of 3 bookmarks', function ($name, $bookmarks) {
    // Given
    $bookmarkGroup = BookmarkGroup::factory()->create([
        'name' => $name,
    ]);
    ...
    $component->assertSeeInOrder([
        ...
        ->assertDontSee($bookmarks[3])
        ->assertDontSee($bookmarks[4])
        ->assertDontSee($bookmarks[5])
        ->assertSee('...');
})->with([
    ['Microsoft Office', ['Teams', 'Outlook', 'Word', 'Calendar', 'PowerPoint', 'OneNote']],
]);
